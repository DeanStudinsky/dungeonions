# Legacy Versions (historical notes)

> ⚠️ **This file is history, not instructions.** It was the old root `CLAUDE.md` and it
> describes a nested layout (`Itsy Bitsy Battling/DangerousDungeonions-Phaser/`,
> `public/old-p5-version/`) that **no longer exists in this repository**. The Phaser game
> now lives flat at the repository root.
>
> For current guidance see **`CLAUDE.md`**. The paths below refer to local-only folders
> (`dungeonions-prePhaser/`, `DangerousDungeonions-Unity/`) that were never pushed here;
> they are kept only to record where the earlier builds came from.

## Project Overview

**Dangerous Dungeonions** is a roguelike dungeon crawler featuring procedural generation, multiple character classes, and an in-game map editor. Development moved through several frameworks (p5.js → Phaser 3, plus a Unity experiment); the notes below cover those earlier stages.
