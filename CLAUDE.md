...
**IMPORTANT - Active Codebase**: `src/` (Phaser 3 game - edit this)

The game is served from the repository root: `index.html` at the root loads everything in
`src/` via plain `<script>` tags in dependency order. There is no build step and no
`public/` directory.

The p5.js version this was migrated from is **not in this repository** — it lives locally in
`dungeonions-prePhaser/`. See `LEGACY-VERSIONS.md` for notes on the older builds.
...
### Entry Point
`index.html` loads Phaser 3 from CDN and includes all game scripts in dependency order.
...
**Two Codebases:**
- `src/` - Active Phaser 3 game (EDIT THIS)
...
**PWA Support:** Service worker (`sw.js`) caches all game assets for offline play. Installable on iOS/Android.

## Common Workflows

### Adding a New Enemy Type
1. Add tile index to `TileConfig.enemies` in [src/config/TileConfig.js](src/config/TileConfig.js)
...
4. Update [src/scenes/MenuScene.js](src/scenes/MenuScene.js) character selection UI
5. Add case to [src/scenes/GameScene.js](src/scenes/GameScene.js) `switch(this.characterType)` block
6. Add `<script>` tag to [index.html](index.html) in entities section

### Modifying the Map
Edit `MapManager.createMap()` in [src/managers/MapManager.js](src/managers/MapManager.js):
...
3. Reimplement in appropriate Phaser entity/scene in `src/`
...
- Location: `assets/tilesets/kenney_tiny-dungeon/Tilemap/tilemap_packed.png`
...
- Location: `assets/tilesets/kenney_roguelike-rpg-pack/roguelikeSheet_transparent.png`
...
The game is deployed to two places, both serving straight from the repository root.

### GitHub Pages
- **URL**: https://deanstudinsky.github.io/dungeonions/
- **Source**: `main` branch, `/` (root) — publishes automatically on push
- `.nojekyll` at the root stops Jekyll from filtering asset directories

### Firebase Hosting
- **Public directory**: `.` (the repository root)
- **Config**: `firebase.json`
- **Deploy**: `firebase deploy` (or `firebase deploy --only hosting`)
- **Local preview**: `firebase serve` (port 5000)

### ⚠️ Keep paths relative

Firebase serves the game at a domain root, GitHub Pages serves it under the
`/dungeonions/` subpath. Any root-absolute URL (`/sw.js`, `/index.html`, `/assets/...`)
works on Firebase but **404s on Pages**. Always use relative paths (`./sw.js`,
`src/main.js`) in `index.html`, `manifest.json`, and `sw.js`.
...
**Script Load Order:**
Script tags in [index.html](index.html) must maintain dependency order:
